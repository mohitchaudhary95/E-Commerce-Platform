    ...
    : IRequestHandler<RemoveCartItemCommand, CartDto>
{
        ...
        RemoveCartItemCommand request, CancellationToken cancellationToken)
    {
        ...
        // Direct DELETE — no change tracker
        await _cartRepository.RemoveCartItemAsync(item.Id, cancellationToken);
        await _cartRepository.UpdateAsync(cart, cancellationToken);
 
        var updated = (await _cartRepository.GetByUserIdAsync(
            request.UserId, cancellationToken))!;
        return CartMapper.MapToDto(updated);
    }
}
    ...
    : IRequestHandler<UpdateQuantityCommand, CartDto>
{
        ...
        UpdateQuantityCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            // Direct UPDATE — no change tracker
            await _cartRepository.UpdateCartItemQuantityAsync(
                item.Id, request.Dto.Quantity, cancellationToken);
        }
    ...
    : IRequestHandler<ClearCartCommand, bool>
{
        ...
        ClearCartCommand request, CancellationToken cancellationToken)
    {
        ...
        // Direct bulk DELETE of all items
